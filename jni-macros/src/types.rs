...
/// `Reference` type.
///
/// E.g. `"jint"` => `{ Primitive(Int), jni::sys::jint }`,
///     `"i32"` => `{ Primitive(Int), i32 }`,
///     `"JString"` => `Reference("jni::objects::JString")`
///
/// ## Reference Types
///
...
/// ## Primitive Types
///
/// A primitive type is represented by the `PrimitiveType` enum, and a full
/// path to the `jni::sys` primitive type (e.g., "jni::sys::jint") or the
/// name of an equivalent Rust primitive type (e.g., "i32").
///
/// Each JNI primitive typically has three primitive RustTypes:
/// - The `jni::sys` type (e.g., "jni::sys::jint")
/// - The Rust primitive type (e.g., "i32")
/// - The Java type name (e.g., "int")
///
/// and one "alias" for each of those.
///
/// A `type_map` may add additional aliases for the `jni::sys::jlong` primitive types,
///
...
pub struct TypeMappings {
    ...
    ///
    /// For now, only built-in types have multiple aliases, so signatures can
    ...
    ///
    /// Only includes the canonical Rust type paths, so make sure to resolve aliases like "JString"
    /// before looking up in this map.
