...
use crate::{
    ...
    types::sig_type_to_rust_type_core,
};
use crate::{
...
#[derive(Clone)]
enum NativeMethodExport {
    /// Use the default behavior from the top-level `export_native_methods` setting
    Default,
...
/// Returns None if no visibility specifier is present (defaults to Public)
fn parse_visibility(input: ParseStream) -> Result<Option<VisibilitySpec>> {
    // Check for 'priv' keyword (reserved keyword, so we check for the ident "priv")
    ...
    if lookahead.peek(Token![pub]) {
        ...
        return Ok(Some(VisibilitySpec::Public));
    }

    // Check for 'priv' keyword (reserved keyword)
    // We need to use parse_any to handle reserved keywords
    let fork = input.fork();
    #[allow(clippy::collapsible_if)]
    if let Ok(ident) = fork.call(syn::Ident::parse_any) {
        if ident == "priv" {
            // Consume it from the actual input
...
fn parse_method(
    ...
    if input.peek(syn::token::Paren) {
        ...
        // For shorthand syntax, if extern was specified, it means export = true
        if is_extern {
        ...
        while !body_content.is_empty() {
            ...
            if lookahead.peek(name) {
                ...
                let key = body_content.parse::<Ident>()?;
                body_content.parse::<Token![=]>()?;
                ...
                match key.to_string().as_str() {
                    ...
                    _ => {
                        return Err(syn::Error::new(key.span(), "Unknown method property"));
                    }
                }
            } else {
        ...
        // For block syntax, if extern was specified before the name, it means export = true
        if is_extern {
            if export.is_some() {
                return Err(syn::Error::new(
                    rust_name.span(),
    ...
    match kind {
        ...
        MethodKind::Method => {
            // Methods cannot be extern (only native methods can)
            if is_extern {
                return Err(syn::Error::new(
                    rust_name.span(),
                    "Only native methods can be 'extern'",
                ));
            }
        }
        MethodKind::NativeMethod => {
            ...
            if is_raw && error_policy.is_some() {
                return Err(syn::Error::new(
                    rust_name.span(),
...
fn parse_field(input: ParseStream, type_mappings: &TypeMappings) -> Result<Field> {
    ...
    let mut getter_name = rust_name.clone();
    ...
    let mut setter_name = format_ident!(
        "{}set_{}",
    ...
    if input.peek(Token![:]) {
        ...
        let mut getter_attrs = field_attrs.clone();
        let mut setter_attrs = field_attrs.clone();
        ...
        while !body_content.is_empty() {
            ...
            if body_content.peek(get) {
                ...
                body_content.parse::<Token![=]>()?;
                getter_name = body_content.parse::<Ident>()?;
                ...
                body_content.parse::<Token![=]>()?;
                setter_name = body_content.parse::<Ident>()?;
            } else if vis.is_some() {
...
pub fn bind_java_type_impl(input: TokenStream) -> Result<TokenStream> {
    ...
    let (static_method_id_fields, static_method_id_inits) =
        generate_method_ids(&input.static_methods, &input.type_mappings, jni)?;

    // Generate field ID fields and initialization code for fields
    let (field_id_fields, field_id_inits) =
        generate_field_ids(&input.fields, &input.type_mappings, false, jni)?;
    let (static_field_id_fields, static_field_id_inits) =
        generate_field_ids(&input.static_fields, &input.type_mappings, true, jni)?;

    // Combine all ID fields and inits
    let all_id_fields = [
        constructor_method_id_fields,
        method_id_fields,
        static_method_id_fields,
        field_id_fields,
        static_field_id_fields,
    ]
    .concat();

    let all_id_inits = [
        constructor_method_id_inits,
        method_id_inits,
        static_method_id_inits,
        field_id_inits,
        static_field_id_inits,
    ]
    .concat();
    ...
    let (native_trait_and_wrappers, native_registration_code) = generate_native_methods_code(
        ...
        &input.static_native_methods,
        &input.native_trait_name,
        &input.type_mappings,
        input.export_native_methods,
        &input.default_error_policy,
        jni,
    )?;

    // Generate type mapping runtime checks
    let _type_mapping_runtime_checks =
        generate_type_mapping_checks(&input.type_mappings, &java_class_dotted, jni);
    ...
    let static_methods_impl =
        generate_methods(&api_name, &input.static_methods, &input.type_mappings, jni)?;

    // Generate field implementations
    let fields_impl = generate_fields(&api_name, &input.fields, &input.type_mappings, false, jni);

    let static_fields_impl = generate_fields(
        &api_name,
        &input.static_fields,
        &input.type_mappings,
        true,
    ...
    // Wrap static methods and fields in impl block
    // Note: Static methods/fields are still callable on any instance lifetime,
    // they just don't use &self. They're not tied to 'static (global references).
...
fn generate_api_get_method(
    ...
    let type_mapping_runtime_checks = generate_type_mapping_checks(type_mappings, java_class, jni);
...
/// Generate runtime assertions that type mappings are correct
///
/// Generates assertions that verify the Java class names match at runtime.
///
...
fn generate_type_mapping_checks(
    type_mappings: &TypeMappings,
    _primary_class: &str,
    jni: &syn::Path,
) -> TokenStream {
...
fn generate_constructors(
    ...
    for constructor in constructors {
        ...
        let arg_lifetimes: Vec<_> = (0..constructor.method_signature.parameters.len())
            .map(|i| {
        ...
        let arg_names: Vec<_> = constructor
            .method_signature
            .parameters
            .iter()
        ...
        let arg_types: Vec<_> = constructor
            .method_signature
            .parameters
            .iter()
            .zip(arg_lifetimes.iter())
            .map(|(param, lt)| sig_type_to_rust_arg_type(&param.ty, lt, type_mappings, jni))
            .collect();

        // Validate that we can generate JNI signature (return error if not)
        constructor
            .method_signature
            .to_jni_signature(type_mappings)
            .map_err(|e| {
        ...
        // Generate jvalue conversion for each argument (calling .as_jni() directly)
        let jvalue_conversions: Vec<_> = constructor
            .method_signature
            .parameters
            .iter()
            .zip(arg_names.iter())
            .map(|(param, name)| {
                if let Some(primitive) = param.ty.try_as_primitive(type_mappings) {
                    // For primitive types, we can use the core conversion directly
        ...
        let lifetime_decls = if arg_lifetimes.is_empty() {
            quote! { <'env_local> }
        } else {
            let lts = &arg_lifetimes;
            quote! { <'env_local, #(#lts),*> }
        };

        // Generate the constructor implementation
        let arg_decls: Vec<_> = arg_names
            .iter()
            .zip(arg_types.iter())
...
fn generate_methods(
    ...
    for method in methods {
        ...
        // Generate unique lifetime for each argument
        let arg_lifetimes: Vec<_> = (0..method.method_signature.parameters.len())
            .map(|i| {
                let lt_name = format!("local_{}", i);
                let lt = syn::Lifetime::new(&format!("'{}", lt_name), Span::call_site());
                quote! { #lt }
        ...
        let jvalue_conversions: Vec<_> = method
            .method_signature
            .parameters
            .iter()
            .zip(arg_names.iter())
            .map(|(param, name)| {
                if let Some(prim) = param.ty.try_as_primitive(type_mappings) {
                    let converter = match prim {
                        PrimitiveType::Boolean => quote! { #jni::objects::JValue::Bool },
                        PrimitiveType::Byte => quote! { #jni::objects::JValue::Byte },
                        PrimitiveType::Char => quote! { #jni::objects::JValue::Char },
                        PrimitiveType::Short => quote! { #jni::objects::JValue::Short },
...
/// Generate field implementations (instance or static fields)
/// Generate field getter/setter implementations (without the impl block wrapper)
fn generate_fields(
    api_name: &Ident,
    fields: &[Field],
    type_mappings: &TypeMappings,
    ...
    for field in fields {
        ...
        let getter_attributes = if !field.getter_attrs.is_empty() {
            &field.getter_attrs
        } else {
            &field.attrs
        };

        // Get visibility for getter and setter
        let getter_visibility = field.getter_visibility.to_tokens();
        let setter_visibility = field.setter_visibility.to_tokens();

        // For setter attributes logic, we need to check and decide what to use
        let use_field_attrs_for_setter = field.setter_attrs.is_empty() && !field.attrs.is_empty();
        let has_doc_in_field_attrs = field.attrs.iter().any(|attr| attr.path().is_ident("doc"));

        // Check if we need to generate default setter doc
        let setter_default_doc = if use_field_attrs_for_setter && has_doc_in_field_attrs {
            let java_name = &field.java_name;
            ...
            quote! { #(#attrs)* }
        } else if use_field_attrs_for_setter && !has_doc_in_field_attrs {
            // If no doc attrs, use the field's attrs for the setter too
    ...
    // Return just the field implementations without the impl block wrapper
    // The impl block will be created by the caller
    quote! {
        #(#field_impls)*
    }
...
fn generate_native_methods_code(
    ...
    static_native_methods: &[NativeMethod],
    native_trait_name: &Option<Ident>,
    type_mappings: &TypeMappings,
    export_native_methods: bool,
    default_error_policy: &Option<syn::Path>,
    ...
    if native_methods.is_empty() && static_native_methods.is_empty() {
    ...
    let registration_code = generate_native_registration_code(
        api_name,
        native_methods,
        static_native_methods,
        type_mappings,
...
fn generate_native_trait(
    ...
    for method in static_native_methods {
        // Skip methods with direct function bindings - they bypass the trait
        if method.native_fn.is_some() {
            continue;
        }
...
fn generate_native_wrappers(
    ...
    // Generate instance method wrappers
    for method in native_methods {
...
fn generate_single_native_wrapper(
    ...
    let wrapper_body = match (&method.native_fn, method.is_raw) {
        // Case 1: Direct raw function (fn = raw path)
        (Some(raw_fn), true) => {
        ...
        // Case 2: Direct safe function (fn = path)
        (Some(safe_fn), false) => {
        ...
        // Case 3: Trait-based raw method (raw = true, no fn)
        (None, true) => {
        ...
        // Case 4: Trait-based safe method (default: no fn, raw = false)
        (None, false) => {
...
fn generate_native_registration_code(
    ...
    // Generate descriptors for instance methods
    for method in native_methods {
        let java_name = &method.java_name;
        let rust_name = &method.rust_name;
    ...
    // Generate descriptors for static methods
    for method in static_native_methods {
        let java_name = &method.java_name;
        let rust_name = &method.rust_name;
        ...
        let jni_signature = method
            ...
            .map_err(|e| {
                syn::Error::new(
                    rust_name.span(),
                    format!(
                        "Failed to generate JNI signature for static native method '{}': {}",
                        rust_name, e
                    ),
                )
            })?;
...
fn generate_native_exports(
    ...
    native_methods: &[NativeMethod],
    static_native_methods: &[NativeMethod],
    global_export_native_methods: bool,
    type_mappings: &TypeMappings,
    jni: &syn::Path,
) -> Result<TokenStream> {
    let mut exports = Vec::new();

    // Generate exports for instance methods
    for method in native_methods {
        // Determine if this method should be exported
        let should_export = match &method.export {
            NativeMethodExport::Default => global_export_native_methods,
    ...
    // Generate exports for static methods
    for method in static_native_methods {
        // Determine if this method should be exported
        let should_export = match &method.export {
            NativeMethodExport::Default => global_export_native_methods,
