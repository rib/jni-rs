...
pub fn test_generated_alias_methods() {
    ...
    let _: Result<()> = attach_current_thread(|env| {
        ...
        let list2 = unwrap(JList::cast_local(list_object2, env), env);
        ...
        let set = unwrap(JSet::cast_local(set_object, env), env);
        ...
        let set2 = unwrap(JSet::cast_local(set_object2, env), env);
